...
export function EscrowRecord({ receipt }: { readonly receipt: unknown }) {
  ...
  if (view === null) return <div><p className="skill-state">Escrow evidence unavailable</p>
  ...
  return <div>
    <p className={`skill-state${e.state === "settled" ? " is-settled" : e.state === "refunded" ? " is-unresolved" : ""}`}>Hub reports escrow {e.state}</p>
