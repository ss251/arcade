...
- Renewer is replaced by the same owned serving runner without the daemon key before the price/expiry beats. The 60s ENS watcher must expose the matching name in the catalogue (bounded 90s wait); final expiry requires matching detail200 `ensExpired` proof while both services remain alive.
