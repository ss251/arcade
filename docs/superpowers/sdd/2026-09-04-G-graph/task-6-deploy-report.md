...
- `prepared` is durably written before key lookup; `dispatch_intent` before at most one POST. Deadline/cancellation checks occur before and after durable IO, at key-reader entry, and at actual fetch entry. A canceled/late key cannot trigger a later POST.
