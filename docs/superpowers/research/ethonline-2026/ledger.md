...
- **DMK signer: no, strictly.** `@ledgerhq/device-transport-kit-speculos` exists and is labelled "**Simulator (dev/test only)**" in Ledger's own skill (`dmk-sdk-reference.md` line 22). Speculos runs the real `app-ethereum` binary (https://github.com/LedgerHQ/speculos), so `getAddress`/`signTypedData` are exercised end-to-end.
