...
| Attester-filtered feedback count shown in `/listings` and MCP describe | yes, display only | no (score-gating with extra steps) | yes, labelled "settlement evidence" | **Display only, labelled, never affects price or routing.** Grok's objection was to gating; display with the label satisfies it. |
