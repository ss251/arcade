...
| **Residual** | **Low.** An injected agent can spend `maxSubSpendUsd` and not a cent more, because it never holds the means to. What remains is a seller whose own skill goes looking for the socket — but that is their machine and their wallet, and a seller "stealing" from themselves is not a threat. An earlier version put the key in the sandbox and enforced the budget there, which made the budget advisory and the wallet balance the only real cap; conflating that seller-side case with the injection case made the residual read worse than it was, and fixing the design let it be stated honestly as Low. |
