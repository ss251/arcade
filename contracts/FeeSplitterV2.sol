 ...
 *         and without any privileged party able to redirect it. v2 adds `settleWithTree`,
 *         which additionally commits the hash of the receipt tree the hub settled under this
 *         authorization — see `SettledTree` below.
 *
 * @dev WHY THIS EXISTS
 *
 ...
 *
 * Consequently `settle`/`settleWithTree` need no access control at all and are
 * permissionless — there is no settler role to compromise, and nothing an attacker gains by
 * calling either, since the only possible destination is the seller the buyer already chose
 * by paying this address.
 *
 * @dev FEES ARE PULLED, NOT PUSHED
 *
 * USDC can blacklist addresses. If settlement pushed the fee to an immutable `treasury` and
...
contract FeeSplitterV2 {
    ...
    /**
     * @dev Shared body of `settle` and `settleWithTree`: redeem the buyer's signed
     *      authorization, pull the funds, split them, and return the split so each external
     *      entry point can emit its own event. Permissionless by construction — the
     *      destination is fixed, so there is nothing to steal by calling this and no role to
     *      compromise. Replay protection is delegated to the token, which marks each
     *      (authorizer, nonce) pair used and reverts on reuse; we deliberately do not
    ...
    function _settle(
        ...
        bytes32 r,
        bytes32 s
    ) internal returns (uint256 sellerAmount, uint256 feeAmount) {
    ...
    function settle(
        ...
        (uint256 sellerAmount, uint256 feeAmount) = _settle(from, value, validAfter, validBefore, nonce, v, r, s);
     ...
     * @dev Separated from settlement precisely so a blacklisted treasury cannot brick
     *      payments.
     */
    ...
    /// @notice Contract version, so callers can distinguish v1 (revert / no selector) from v2.
    function version() external pure returns (uint8) {
        return 2;
    }

    /**
     * @notice USDC sitting here beyond `accruedFees` — i.e. sent by mistake outside
     *         settlement.
