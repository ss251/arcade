 ...
 * requirements refuse the inventory. Escrow is reserved, not an exact signer.
...
export const paymentChoices = (accepts: readonly unknown[], preferRail?: readonly ListingRail[]): readonly PaymentChoice[] => {
  try {
...
export const selectAccept = (accepts: readonly unknown[], preferRail?: readonly ListingRail[], gatewayBalanceAtomic?: bigint): PaymentChoice | undefined => {
  ...
  return paymentChoices(accepts, preferRail).find(choice => choice.rail !== "gateway" ||
